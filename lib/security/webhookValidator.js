...
export function validateHotmartPayload(payload) {
    ...
    // Eventos de cancelamento/reembolso NÃO exigem product/buyer
    ...
    const isCancellationEvent = cancellationEvents.includes(payload.event);

    if (!isCancellationEvent && payload.data && !payload.data.product) {
        errors.push('Campo "data.product" ausente');
    }

    if (!isCancellationEvent && payload.data && !payload.data.buyer) {
